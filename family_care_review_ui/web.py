...
const esc=v=>String(v??'nicht vorhanden').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const display=x=>typeof x==='object'?Object.values(x).flat().join(' · '):x;
const list=a=>a&&a.length?`<ul>${a.map(x=>`<li>${esc(display(x))}</li>`).join('')}</ul>`:'<p>nicht vorhanden</p>';
async function load(){
  const s=await fetch('/api/state').then(r=>r.json());
  document.querySelector('#notice').textContent=s.case.notice;
  document.querySelector('#progress').textContent=s.guardian_view.progress;
  document.querySelector('#next').disabled=!s.conversation.can_advance;
  const c=s.source_chain;
  document.querySelector('#app').innerHTML=`<div class="grid">
    <section class="card wide"><h2>${esc(s.journey.heading)}</h2><p>${esc(s.guardian_view.summary)}</p>${s.conversation.current_question?`<div class="question"><span class="label">Aktuelle Guardian-Frage</span>${esc(s.conversation.current_question)}</div>`:'<div class="question">Keine aktuelle Guardian-Frage.</div>'}</section>
    <section class="card"><h2>Was wir bereits wissen</h2>${list(s.guardian_view.known)}</section>
    <section class="card"><h2>Was noch offen ist</h2>${list(s.guardian_view.open)}</section>
    <section class="card"><h2>Was als Nächstes geprüft werden sollte</h2>${list(s.guardian_view.next_checks)}</section>
    <section class="card"><h2>Wer einbezogen werden muss</h2>${list(s.guardian_view.involved)}</section>
    <details class="card wide"><summary><strong>Prüfansicht · Details anzeigen</strong></summary><div class="grid">
      <section class="card"><h2>Understanding</h2><h3>Facts</h3>${list(s.understanding.facts)}<h3>Hypotheses</h3>${list(s.understanding.hypotheses)}<h3>Unknowns</h3>${list(s.understanding.unknowns)}<h3>Contradictions</h3>${list(s.understanding.contradictions)}<h3>Goals</h3>${list(s.understanding.goals)}</section>
      <section class="card"><h2>Cross-Domain</h2><h3>Contributions</h3>${list(s.cross_domain.contributions)}<h3>Dependencies</h3>${list(s.cross_domain.dependencies)}<h3>Personen und Rollen</h3>${list(s.cross_domain.people)}<h3>Dokumentreferenzen</h3>${list(s.cross_domain.documents)}</section>
      <section class="card"><h2>Journey und Review</h2><h3>Status</h3><p>${esc(s.journey.status)}</p><h3>Beantwortet</h3>${list(s.cross_domain.answered_points)}<h3>Offen oder zurückgestellt</h3>${list([...s.cross_domain.open_points,...s.cross_domain.deferred_points])}<h3>Professional Reviews</h3>${list(s.cross_domain.reviews)}<h3>Organisatorische Schritte</h3>${list(s.cross_domain.steps)}</section>
      <section class="card"><h2>Fachliche Grenzen</h2>${list(s.journey.boundaries)}<h3>Zulässige Aktionen</h3>${list(s.journey.allowed_actions)}</section>
      <section class="card wide"><h2>Quellenkette des letzten Schritts</h2><div class="chain">${Object.entries(c).map(([k,v])=>`<div><span class="label">${esc(k)}</span>${esc(Array.isArray(v)?v.join(', '):v)}</div>`).join('')}</div><details><summary>Technische Referenzen</summary><pre>${esc(JSON.stringify(s.debug,null,2))}</pre></details></section>
    </div></details>
  </div>`
}
async function post(path){await fetch(path,{method:'POST'});await load()}
document.querySelector('#next').onclick=()=>post('/api/advance');document.querySelector('#reset').onclick=()=>post('/api/reset');load();</script></body></html>'''
